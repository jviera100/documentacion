...
### 177. (Pregunta 6) Modificación de Prototipos `Array.prototype.map` (Incorrecta – No usa el callback)(contenido complementario ###34)
...
### 178. (Pregunta 7) Mutación de Arrays por índice y .push.bind()
...
- **Hoisting (Elevación):** Las declaraciones de `function` y `var` se mueven al inicio de su ámbito antes de la ejecución. No funciona con `let` y `const`.
...
**Análisis de la Solución:**
- `clone` y `clone2` **comparten** el mismo objeto `c`, así que **si cambias `obj.c.deep` también cambia en ellos.**
- `clone3` es **independiente** porque se reconstruyó completamente (Deep Copy), por eso **conserva el valor original.** 
...
-La función `pipe` ejecuta las funciones en secuencia de izquierda a derecha sobre el valor de entrada `4`:
...
- **Función Pura:** Aquella que cumple dos condiciones:
  1. **determinista**: Siempre devuelve el mismo resultado para los mismos argumentos.
  2. **No causa efectos secundarios** (no modifica nada fuera de sí misma, como imprimir en consola, cambiar una variable externa, acceder al tiempo, etc.).
...
### 192 **🔄 Callback vs 🔐 Closure vs ⏳ Promise**

📌 Definiciones

- **Callback**:  
  Función que se **pasa como argumento** a otra función.  
  Se ejecuta después de cierto evento o acción (`console.log, setTimeout, setInterval, forEach, map, filter, reduce, addEventListener, .then()`).

- **Closure**:  
  Función que **recuerda el entorno** donde fue creada,  
  incluso si se ejecuta fuera de ese contexto (`return`).

- **Promise**:  
  Objeto que **espera** una operación asíncrona y  
  luego ejecuta un callback cuando termina (`new Promise, resolve, reject, .then(), .catch(), .finally() y async/await`).
  
✅**¡Y pueden coexistir al mismo tiempo!**
...
crearSaludo()();
```
✅ Promise (con callback y closure)
